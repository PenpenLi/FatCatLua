...
function ToolCom.seekNodeByName(node,name)
	...
	for idx, child in pairs(node:getChildren()) do
		...
		local res = ToolCom.seekNodeByName(child,name)
...
function ToolCom.ButtonArmature(button, btnName, armatureFile,isbk)
	local size = button:getContentSize()
	local am = ccs.Armature:create(armatureFile)
	am:setPosition(cc.p(size.width*0.5, size.height*0.5))
	am:getAnimation():play("Animation1")
	button:addChild(am)
	
	if isbk then
		local bk = display.newSprite("res/btnText/bg.png")
		bk:setPosition(cc.p(size.width*0.5, 0))
		bk:setAnchorPoint(Vec2(0.5, 0))
		button:addChild(bk)
	end
	local btnNamePath = string.format("res/btnText/%s.png",btnName)
	
	local tex = display.newSprite(btnNamePath)
	tex:setPosition(cc.p(size.width*0.5, size.height*0.06))
	tex:setAnchorPoint(cc.p(0.5, 0))
	tex:setScale(1.1)
	tex:setName(btnName)
	button:addChild(tex)
end
